# お知らせ
- 2022/04/02 11:20,復旧したようです。  
  ただし、エイプリルフール中にダウンロードした譜面や playlist 内の曲が表示されない場合、変なデータが beatlist 内に残ってしまっていることが原因の可能性があります。  
  その場合は、[Settings] 画面から [CLEAR CACHE] & [UPDATE LIBRARY] を実行してみてください。  
  ※ちゃんと表示されている場合、または beatsaver.com のエイプリルフール期間中(4/1 (時間不明)から 4/2 11:20頃にかけて)に  
    一度も beatlist を起動していない場合は気にする必要はありません。
- ~~2022/04/01,beatsaver.com のエイプリルフールの影響により[Beatmap Online]画面が使えなくなっています。~~  
  ~~日本時間でエイプリルフールが終わっても海外はまだ期間中のため、もう少し続くかもしれません。~~  
  ~~4/3以降なら復帰するのではないかと思います。~~
---
