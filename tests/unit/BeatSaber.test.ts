...
describe("beatSaber", () => {
  ...
  it("should get all folder (supposed to be beatmap) in the folder", async () => {
    ...
    expect(first).toBe(
      path.join(
        __dirname,
        "../data/fakeInstallation/Beat Saber_Data/CustomLevels/bar"
      )
    );
    expect(second).toBe(
      path.join(
        __dirname,
        "../data/fakeInstallation/Beat Saber_Data/CustomLevels/foo"
      )
    );
  });
  ...
  it("should find all playlists file", async () => {
    ...
    expect(second).toBe(
      path.join(__dirname, "../data/fakeInstallation/Playlists/file2.json")
    );
